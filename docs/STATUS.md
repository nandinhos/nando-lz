...
A `main` está publicada em produção: CI, deploy via SSH restrito e health check HTTPS passaram. A manutenção usa PRs autônomas de escopo restrito, com merge e deploy somente depois dos gates independentes.
...
- `autonomous-merge.yml` não faz checkout de PRs: valida origem/branch, escopo do diff, os resultados mais recentes de PHP 8.3/8.4, auditoria de dependências e GitGuardian antes de solicitar merge por rebase com SHA fixado. Ele aguarda a publicação do commit rebaseado antes de disparar nova CI.
